...
public class SpotifyTokenService {
    ...
    public SpotifyTokenTable setAccessToken(Long userId, String token) {
        Optional<SpotifyTokenTable> optionalTokenTable = spotifyTokenRepository.findById(userId);
    
        if (optionalTokenTable.isPresent()) {
            SpotifyTokenTable tokenTable = optionalTokenTable.get();
            tokenTable.setAccessToken(token);
            // No need to set userId again as it remains the same
            return spotifyTokenRepository.save(tokenTable);
        } else {
            // Create a new entity for the given userId
            SpotifyTokenTable newTokenTable = new SpotifyTokenTable();
            newTokenTable.setUserId(userId);
            newTokenTable.setAccessToken(token);
            return spotifyTokenRepository.save(newTokenTable);
        }
    }
    
    public SpotifyTokenTable setAuthorizationToken(Long userId, String token) {
        Optional<SpotifyTokenTable> optionalTokenTable = spotifyTokenRepository.findById(userId);
        ...
        if (optionalTokenTable.isPresent()) {
            ...
            SpotifyTokenTable newTokenTable = new SpotifyTokenTable();
            newTokenTable.setUserId(userId);
            newTokenTable.setAuthToken(token);
            return spotifyTokenRepository.save(newTokenTable);
        }
    }
